...
import com.esprit.android.inart.adapters.ImageGallerySubcategoryAdapterEvent;
...
public class MyEventList extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
			...
			public void done(List<ParseObject> conThus, ParseException e) {
				if (e == null) {
					...
					for (int i = 0; i < conThus.size(); i++) {
						...
						mang.add(new EventElement(
								dong.getString("EventName"),
								dong.getString("Description"),
								dong.getParseFile("EventImage"),
								dong.getParseFile("EventImage").getUrl(),
								dong.getString("Owner"),
								dong.getString("Address"),
								dong.getString("EventDate")


						));
					}
					BaseAdapter adapter = new ImageGallerySubcategoryAdapterEvent(getBaseContext(), (ArrayList<EventElement>) mang, mIsLayoutOnTop);
        ...
        System.out.println(mang.size() + " this is size");
			...
			private void setAdapter() {
				BaseAdapter adapter = new ImageGallerySubcategoryAdapterEvent(this, (ArrayList<EventElement>)mang, mIsLayoutOnTop);
				mListView.setAdapter(adapter);
			}
		}
