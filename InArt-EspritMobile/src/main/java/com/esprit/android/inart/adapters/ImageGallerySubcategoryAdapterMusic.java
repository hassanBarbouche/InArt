...
public class ImageGallerySubcategoryAdapterMusic extends BaseAdapter /*implements OnClickListener*/ {
	...
	private ArrayList<MusicElement> mImageGallerySubcategories;
	private final boolean mIsLayoutOnTop;

	public ImageGallerySubcategoryAdapterMusic(Context context,
											   ArrayList<MusicElement> imageGallerySubcategories, boolean isLayoutOnTop) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (type == TYPE_ONE_COLUMN) {
			...
			MusicElement model1 = mImageGallerySubcategories.get(position * 2);
			...
			ImageUtil.displayImage(oneColumnViewHolder.image1, model1.url_photo, null);
			...
			if (convertView == null) {
				convertView = mInflater.inflate(R.layout.list_item_categories_two_columns_music, parent, false);
			...
			ImageUtil.displayImage(twoColumnsViewHolder.image1, model1.url_photo, null);

			MusicElement model2 = mImageGallerySubcategories.get(position * 2 + 1);
			...
			ImageUtil.displayImage(twoColumnsViewHolder.image2, model2.url_photo, null);
